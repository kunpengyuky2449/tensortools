...
target_folder = 'D:\BCM_projects\TCA_project2\SAAAT_preprocess_matlab\GBGN_to_concat';
...
for i = 1: numel(tensor_file_name)
    ...
    new_N = new_N + size(Session_tensor,1);
    new_K = new_K + size(Session_tensor,3);
    old_T = size(Session_tensor,2);
    clear Session_tensor
end
%%
pooled_mask = false(new_N, old_T, new_K,'logical');
current_N = 0;
current_K = 0;
for i = 1: numel(tensor_file_name)
    load(fullfile(target_folder,tensor_file_name{i}),"Session_tensor")
    ...
    pooled_mask(current_N+1:Next_N, :, current_K+1:Next_K) = true;
    current_N = Next_N;
    current_K = Next_K;
end
