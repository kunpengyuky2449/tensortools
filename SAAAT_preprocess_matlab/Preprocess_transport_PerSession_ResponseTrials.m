...
% 4) Not a miss or correct rejection trial (which has no response)
...
for i = 2:numel(file_path_PARAMS)
    ...
    ds_res_start = round(ds_tc_start+Lick_fromTC*1000);
    ...
    for j = 1:numel(ds_res_start)
        ...
        if ds_res_start(j) > time_last_sp
            tbd2(end+1) = j; %#ok
            continue
        end
        ...
        % window -3s to +0s around lick response 
        % To prevent dependency that locomotuion led by rewarding,
        % locomotion after mouse hit-lick wont exlcude the trial
        move_check = sum(...
            walk_checks_temp > ds_res_start(j)/1000-3  & ...
            walk_checks_temp < ds_res_start(j)/1000, ...
            [1,2]);
    ...
    Tensor_PerSession_Summary.ds_res_start = ds_res_start; 
    Tensor_PerSession_Summary.TCdur = TCdur;
    ...
    uniq_neurons = uniq_neurons(cdf_diff < 0.18);
    % Now decide the shape of the tensor
    T_timestamps = numel(5000-2999:5000+3000);  % Trial Window around signal -1000ms to 3000ms
    K_trials = numel(ds_res_start);
    ...
    for n = 1:numel(uniq_neurons)
        ...
        for k = 1:numel(Tensor_PerSession_Summary.ds_res_start)
            rwd_time = Tensor_PerSession_Summary.ds_res_start(k);
            tobeinsert = single(firing_rate_array(rwd_time-2999:rwd_time+3000));
    ...
    file_path = 'D:\BCM_projects\TCA_project2\SAAAT_preprocess_matlab\tensor_PerSession_response';
    name_temp = strcat(file_name,'_signal_tensor_response_summary_', datestr(now,'yymmdd_HH_MM_SS')); %#ok
    save(fullfile(file_path, name_temp),"Tensor_PerSession_Summary")
    name_temp = strcat(file_name,'_signal_tensor_response_NTK_', datestr(now,'yymmdd_HH_MM_SS')); %#ok
