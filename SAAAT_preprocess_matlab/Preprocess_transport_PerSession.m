...
for i = 2:numel(file_path_PARAMS)
    ...
    uniq_neurons = uniq_neurons(Pair_EDist > quantile(Pair_EDist,0.3));
    ...
    Tensor_PerSession_Summary.neuron_depth = depths_with_clus_id( ...
        Pair_EDist > quantile(Pair_EDist,0.3),1);
    ...
    file_path = 'D:\BCM_projects\TCA_project2\SAAAT_preprocess_matlab\tensor_perSession';
    name_temp = strcat(file_name,'_tensor_summary_', datestr(now,'yymmdd_HH_MM_SS')); %#ok
    save(fullfile(file_path, name_temp),"Tensor_PerSession_Summary")
    name_temp = strcat(file_name,'_tensor_NTK_', datestr(now,'yymmdd_HH_MM_SS')); %#ok
